...
#include "led.h"
int ticks=0;
extern int GET_TEMP_SIGN;
extern int PID_CAL_SIGN;
extern int GET_KEY_SIGN;
//V1.1 20120904
//1,增加TIM3_PWM_Init函数。
...
void TIM3_IRQHandler(void)   //TIM3中断
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) //检查指定的TIM中断发生与否:TIM 中断源 
		...
		ticks++;
		if(ticks>=10000) ticks=0; //避免数据溢出
		if(ticks%1==0)		GET_KEY_SIGN=1; //获取按键信息标志位
		if(ticks%100==0) GET_TEMP_SIGN=1; //获取温度信息标志位
		if(ticks%201==0) PID_CAL_SIGN=1;	//PID计算标志位
		}
}
